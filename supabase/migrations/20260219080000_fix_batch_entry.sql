-- Migration: Fix Batch Product Entry (Add User ID to Cost Logic)
-- Description: Updates process_batch_product_entry to pass auth.uid() to process_product_entry_cost.
...
BEGIN
    ...
    LOOP
        ...
        -- FIXED: Added auth.uid() as the 5th argument
        PERFORM process_product_entry_cost(
            ...
            p_vat_percentage,
            auth.uid() -- Pass current user ID for history/audit
        );
