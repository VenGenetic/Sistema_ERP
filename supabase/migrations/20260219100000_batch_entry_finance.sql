-- Migration: Batch Entry Finance Integration (FIXED)
-- Description: Adds "Compra de mercadería" account and updates batch processing logic to include quantity and financial transactions.

-- 0. Ensure unique constraint on inventory_levels for upsert
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'inventory_levels_product_id_warehouse_id_key'
    ) THEN
        ALTER TABLE inventory_levels ADD CONSTRAINT inventory_levels_product_id_warehouse_id_key UNIQUE (product_id, warehouse_id);
    END IF;
END $$;

-- 1. Create "Compra de mercadería" account if it doesn't exist
DO $$
BEGIN
    -- Check by name first
    IF NOT EXISTS (SELECT 1 FROM accounts WHERE name = 'Compra de mercadería') THEN
        -- Check if code is taken, if so, use alternative or just try insert
        IF NOT EXISTS (SELECT 1 FROM accounts WHERE code = '1.1.05') THEN
             INSERT INTO accounts (code, name, category, is_nominal, currency)
             VALUES ('1.1.05', 'Compra de mercadería', 'asset', true, 'USD');
        ELSE
             -- Fallback or log? Just skipping if code exists.
             NULL;
        END IF;
    END IF;
END $$;
...
-- 2. Update process_batch_product_entry
CREATE OR REPLACE FUNCTION process_batch_product_entry(
    p_brand_id BIGINT,
    p_warehouse_id INT,
    p_vat_percentage NUMERIC,
    p_payment_account_id INT, -- NEW: Account to debit (pay from)
...
BEGIN
    ...
    LOOP
        ...
        v_quantity := (v_product_record->>'quantity')::INT; -- NEW

        -- 1. Find or Create Product (Same logic as before)
        ...
        ELSE
            ...
            VALUES (v_sku, v_name, p_brand_id, p_vat_percentage, v_final_margin, 'General') -- Default category
        ...
        -- 2. INVENTORY UPDATE (New Logic)
        -- Update inventory level
        -- NOW SAFE: inventory_levels has UNIQUE (product_id, warehouse_id)
        ...
        -- Accumulate Total Cost for Transaction (Cost * Quantity)
        -- Using Gross Cost (Cost + VAT)
    ...
    IF v_total_transaction_amount > 0 THEN
        ...
        -- Debit (Merchandise Account) - Asset increasing
        INSERT INTO transaction_lines (transaction_id, account_id, credit, debit)
    ...
    RETURN jsonb_build_object(
        'success', true,
        'message', 'Batch processed successfully with financial transaction.',
