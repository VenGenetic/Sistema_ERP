...
Required env vars (`.env`, Vite-exposed): `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`. `GEMINI_API_KEY` is also read by `vite.config.ts` and injected as `process.env.API_KEY`/`process.env.GEMINI_API_KEY`.
...
The repo root has a lot of one-off scripts, exported data dumps, and scratch files (`scripts/`, `scratch/`, `*.xlsx`, `refactor*.py`, `check_db.js`, `tsc_output.txt`, etc.) accumulated from past debugging/migration sessions. These are not part of the app's runtime and can generally be ignored unless a task specifically references one.
