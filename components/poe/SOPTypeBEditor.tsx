  ...
  if (!isEditing) {
    ...
    return (
      <div className="space-y-6 max-w-3xl mx-auto py-4">
        ...
        <div className="flex items-center justify-between p-2 bg-surface-3 rounded-2xl border border-subtle">
          <div className="flex gap-1">
            <button
              onClick={() => setViewMode('WIZARD')}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${ viewMode === 'WIZARD' ? 'bg-primary text-white shadow-md shadow-primary/25' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700' }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${ viewMode === 'MAP' ? 'bg-primary text-white shadow-md shadow-primary/25' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700' }`}
            >
          ...
          {history.length > 0 && viewMode === 'WIZARD' && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-fg-muted hover:text-slate-700 dark:hover:text-slate-200 bg-surface border border-subtle rounded-xl hover:shadow transition-all mr-1"
            >
        ...
        {viewMode === 'WIZARD' && activeNode && (
          <div className="space-y-6">
            ...
            <div className={`p-6 md:p-8 rounded-3xl border shadow-lg transition-all ${ activeNode.node_type === 'QUESTION' ? 'bg-white dark:bg-[#161b22] border-primary/30 shadow-primary/5' : 'bg-success border border-success/40 shadow-success/10' }`}>
              <div className="flex items-center justify-between gap-2 mb-4">
                <span className={`px-3 py-1 rounded-full text-xs font-extrabold tracking-wide font-mono uppercase flex items-center gap-1.5 ${ activeNode.node_type === 'QUESTION' ? 'bg-primary-soft text-primary border border-primary/20 dark:border-primary' : 'bg-success-soft text-success border border-success/20 dark:border-success' }`}>
                ...
                {history.length > 0 && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-fg-muted bg-surface-3 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-all"
                  >
              ...
              <h3 className="text-xl md:text-2xl font-bold text-fg tracking-tight leading-snug">
                {activeNode.question_or_action}
              </h3>

              {activeNode.detail_text && (
                <p className="mt-3 text-sm text-fg-muted leading-relaxed bg-surface-2 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/80">
                  {activeNode.detail_text}
                </p>
              )}
              ...
              {activeNode.node_type === 'QUESTION' && (
                <div className="mt-8 grid grid-cols-2 gap-4">
                  <button
                    ...
                    className="group relative overflow-hidden p-4 rounded-2xl bg-success hover:from-success hover:to-success text-white font-bold text-lg flex items-center justify-center gap-3 shadow-lg shadow-success/25 active:scale-[0.98] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    className="group relative overflow-hidden p-4 rounded-2xl bg-danger hover:from-danger hover:to-danger text-white font-bold text-lg flex items-center justify-center gap-3 shadow-lg shadow-danger/25 active:scale-[0.98] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center group-hover:scale-110 transition-transform">
              ...
              {activeNode.node_type === 'ACTION' && (
                <div className="mt-8 p-4 bg-success/10 border border-success/30 rounded-2xl flex items-center justify-between gap-4">
                  ...
                  <button
                    onClick={handleWizardReset}
                    className="px-4 py-2 bg-success hover:bg-success text-white rounded-xl text-xs font-bold transition-all shadow-sm shrink-0"
                  >
            ...
            {history.length > 0 && (
              <div className="p-4 bg-surface border border-subtle rounded-2xl space-y-2">
                <h5 className="text-xs font-bold font-mono text-fg-subtle uppercase tracking-widest flex items-center gap-1.5">
                ...
                <div className="space-y-1.5">
                  {history.map((step, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between text-xs py-1.5 px-3 bg-surface-2 rounded-lg border border-slate-100 dark:border-slate-800/60">
                        <span className="text-fg font-medium truncate pr-2">
                          {idx + 1}. {n?.question_or_action || 'Pregunta anterior'}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full font-extrabold text-2xs shrink-0 ${ step.answer === 'YES' ? 'bg-success-soft text-success border border-success/20 dark:border-success' : 'bg-danger-soft text-danger border border-danger/20 dark:border-danger' }`}>
        ...
        {viewMode === 'MAP' && (
          <div className="space-y-4">
            <div className="p-4 bg-surface-2 border border-subtle rounded-2xl text-xs text-fg-muted flex items-center gap-3">
            ...
            <div className="space-y-3">
              {nodes.map((node, idx) => {
                ...
                return (
                  <div key={node.id} className="p-4 bg-surface border border-subtle rounded-2xl space-y-3 shadow-2xs">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 font-mono text-2xs font-bold rounded bg-surface-3 text-fg-muted">
                        NODO #{idx + 1}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${ node.node_type === 'QUESTION' ? 'bg-primary-soft text-primary dark:text-primary' : 'bg-success-soft text-success dark:text-success' }`}>
                    ...
                    {node.detail_text && <p className="text-xs text-fg-muted">{node.detail_text}</p>}
                    ...
                    {node.node_type === 'QUESTION' && (
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                        <div className="p-2 rounded-xl bg-success-soft/50 border border-success/20 text-fg">
                          <span className="font-bold text-success block mb-0.5">➡ Si responde SÍ:</span>
                          <span className="line-clamp-1 italic text-[11px]">{yesNode ? yesNode.question_or_action : 'Sin asignar'}</span>
                        </div>
                        <div className="p-2 rounded-xl bg-danger-soft/50 border border-danger/20 text-fg">
                          <span className="font-bold text-danger block mb-0.5">➡ Si responde NO:</span>
    ...
    <div className="space-y-4 max-w-4xl mx-auto py-4">
      <div className="flex items-center justify-between border-b border-subtle pb-3">
        <div>
          ...
          <p className="text-xs text-fg-muted">Define las preguntas de decisión, conecta las ramas de Sí/No y establece las acciones conclusivas.</p>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 font-bold text-xs bg-primary hover:bg-primary text-white rounded-lg transition-all shadow-sm"
        >
      ...
      <div className="space-y-4">
        {nodes.map((node, idx) => {
          ...
          return (
            <div
              key={node.id}
              className={`p-5 bg-surface border rounded-2xl space-y-4 transition-all relative ${ isRoot ? 'border-primary/50 shadow-md shadow-primary/5 ring-1 ring-primary/20' : 'border-slate-200 dark:border-slate-700 shadow-2xs' }`}
            >
              {/* CABECERA DEL NODO */}
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 rounded font-mono text-[11px] font-bold ${ isRoot ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-slate-800 text-fg-muted dark:text-slate-300' }`}>
                  ...
                  <select
                    ...
                    className="text-xs font-bold px-2 py-1 rounded bg-surface-2 border border-subtle text-fg focus:outline-none focus:border-primary"
                  >
                ...
                {!isRoot && (
                  <button
                    ...
                    className="p-1 text-fg-subtle hover:text-danger hover:bg-danger-soft rounded-lg transition-colors"
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full font-bold text-sm px-3 py-2 bg-surface-2 border border-subtle rounded-xl text-fg focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                </div>
                ...
                <div>
                  <textarea
                    ...
                    rows={2}
                    className="w-full text-xs px-3 py-2 bg-surface-2 border border-subtle rounded-xl text-fg focus:outline-none focus:ring-1 focus:ring-primary custom-scrollbar resize-none"
                  />
                </div>
              </div>
              ...
              {node.node_type === 'QUESTION' && (
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  {/* RAMA SÍ (VERDE) */}
                  <div className="p-3.5 bg-success/5 border border-success/30 rounded-xl space-y-2">
                    ...
                    <select
                      ...
                      className="w-full px-2.5 py-2 text-xs bg-surface border border-success/20 rounded-lg text-fg font-medium focus:outline-none focus:ring-1 focus:ring-success truncate"
                    >
                    ...
                    {!node.yes_next_id && (
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 py-1 px-2 text-[11px] font-bold bg-success/10 hover:bg-success text-success hover:text-white rounded-lg transition-all border border-success/30 flex items-center justify-center gap-1"
                        >
                  ...
                  {/* RAMA NO (ROJO/ROSE) */}
                  <div className="p-3.5 bg-danger/5 border border-danger/30 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      className="w-full px-2.5 py-2 text-xs bg-surface border border-danger/20 rounded-lg text-fg font-medium focus:outline-none focus:ring-1 focus:ring-danger truncate"
                    >
                      <option value="">-- Seleccionar Nodo de Destino --</option>
                      {nodes.filter(n => n.id !== node.id).map(n => (
                        <option key={n.id} value={n.id}>
                    ...
                    {!node.no_next_id && (
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 py-1 px-2 text-[11px] font-bold bg-danger/10 hover:bg-danger text-danger hover:text-white rounded-lg transition-all border border-danger/30 flex items-center justify-center gap-1"
                        >
                          <Plus className="w-3.5 h-3.5" /> Nueva Pregunta
                        </button>
                        <button
              ...
              {node.node_type === 'QUESTION' && (node.yes_next_id || node.no_next_id) && (
                <div className="text-[11px] text-fg-subtle pt-1 flex items-center gap-4 font-mono">
